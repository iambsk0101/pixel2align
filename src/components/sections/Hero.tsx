...
export function Hero() {
  ...
  return (
    <EditorPane id="hero" lines={36}>
      <div className="hero-stage relative">
        {/* WebGL background — sits behind content, absolute */}
        <div className="absolute -inset-x-6 md:-inset-x-10 -top-10 h-[min(80vh,720px)] -z-10 overflow-hidden rounded-2xl">
          <HeroShader />
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-editor pointer-events-none" />
        </div>
        ...
        <Reveal>
          <h1 className="hero-title font-display uppercase tracking-tight leading-[0.85] text-[clamp(3rem,12vw,9.5rem)]">
        ...
        <Reveal delay={120}>
          <div className="mt-8 flex flex-wrap gap-2">
            {["Web Designer", "UI Designer", "Conversion Strategist", "@ Studio · 2026"].map((b, i) => (
              <span key={b} className="font-mono text-xs px-3 py-2 rounded border border-border bg-surface/60 backdrop-blur-sm">
                <span
        ...
        <Reveal delay={380}>
          <div className="mt-10 flex flex-wrap gap-3">
            <button onClick={() => openFile("critique")} className="group inline-flex items-center gap-2.5 px-6 py-3 rounded-md bg-accent text-accent-foreground font-mono text-sm font-semibold hover:translate-y-[-2px] transition-transform">
              <Wand2 className="h-4 w-4" /> Free AI Critique
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </button>
            <button onClick={() => openFile("projects")} className="inline-flex items-center gap-2.5 px-6 py-3 rounded-md border border-border bg-surface/80 backdrop-blur-sm font-mono text-sm hover:border-accent transition-colors">
              <FolderGit2 className="h-4 w-4" /> Projects
            </button>
            <button onClick={() => openFile("about")} className="inline-flex items-center gap-2.5 px-6 py-3 rounded-md border border-border bg-surface/80 backdrop-blur-sm font-mono text-sm hover:border-accent transition-colors">
              <User className="h-4 w-4" /> About
            </button>
            <button onClick={() => openFile("contact")} className="inline-flex items-center gap-2.5 px-6 py-3 rounded-md border border-border bg-surface/80 backdrop-blur-sm font-mono text-sm hover:border-accent transition-colors">
              <Mail className="h-4 w-4" /> Contact
            </button>
            <button onClick={openBrief} className="inline-flex items-center gap-2.5 px-6 py-3 rounded-md border border-border bg-surface/80 backdrop-blur-sm font-mono text-sm hover:border-accent transition-colors">
